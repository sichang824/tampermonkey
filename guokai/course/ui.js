...
function createControlPanel() {
  ...
  const panelContent = `
    ...
    <div id="panel-body" class="p-3">
      ...
      <div class="mb-1">当前状态: <span id="current-status" class="font-medium">${window.getCurrentStatus()}</span></div>
      ...
      <div class="grid grid-cols-2 gap-2 mb-3">
        ...
        ${createButton("stop-btn", "停止", "bg-danger hover:bg-danger/80")}
      </div>
      
      <div class="space-y-2">
        ${createCheckbox("auto-process", "自动处理", window.configProxy.autoProcessEnabled)}
        ${createCheckbox("dark-mode", "黑暗模式", window.configProxy.isDarkMode)}
      </div>
    </div>
  `;
...
// 使用Tailwind创建按钮
function createButton(id, text, customClass = "bg-primary hover:bg-primary-hover") {
...
// 更新面板主题（明/暗模式）
function updatePanelTheme(panel, isDarkMode) {
  if (isDarkMode) {
...
function addEventListeners(panel) {
  const eventMap = {
    ...
    "fetch-completeness": { event: "click", handler: window.fetchActivityRead },
    "next-btn": { event: "click", handler: window.clickNextButton },
    "auto-process": {
      event: "change",
      handler: (e) => {
        ...
        if (window.configProxy.autoProcessEnabled) {
          window.executeActionByPageType();
        }
      },
    },
    "dark-mode": {
      ...
      handler: (e) => {
        ...
        updatePanelTheme(panel, e.target.checked);
      },
    },
    "auto-process-btn": {
      event: "click",
      handler: () => {
        window.updateStatus(`手动触发自动处理`);
        window.executeActionByPageType();
      },
    },
    "stop-btn": { event: "click", handler: window.stopProcessing },
    "complete-video": {
      event: "click",
          ...
          .catch((error) => {
            ...
            setTimeout(() => {
              try {
                ...
                window.updateStatus("视频播放失败");
              }
            }, 1000);
          });
      }

      window.updateStatus("正在完成视频播放");
    } catch (error) {
      console.error("设置视频参数失败:", error);
      window.updateStatus("设置视频参数失败");
    }
  } else {
    window.updateStatus("未找到视频元素");
  }
}
...
window.updateCompletenessDisplay = updateCompletenessDisplay;
window.updateAutoProcessStatus = updateAutoProcessStatus;
